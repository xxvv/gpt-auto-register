...
- 在代理分组先输入 `Webshare API Key`，再点击 `获取当前` 或 `替换代理`，扩展会直接调用 Webshare 官方接口，并严格使用接口返回的 `socks5_port` 设置 `socks5` 代理；如果接口没返回 `socks5_port`，会直接报错。`设置代理` 会重新应用当前缓存代理，`清除代理` 只会清除 Firefox 当前代理。
